...
CLEANUP_DIRECTORY_LIST = [
    ...
    path.join(path_to_module, "fake_nextseq_nml_data"),
...
class TestEndToEnd(unittest.TestCase):
    ...
    def test_valid_nextseq_nml_classic_upload(self):
        """
        ...
        # This may not be the project that the files get uploaded to,
        # but one will be made in the case this is the only test being run
        project_name = "test_project_nextseq_nml"
        project_description = "test_project_description_nextseq_nml"
        project = model.Project(name=project_name, description=project_description)
        test_api.send_project(project)
        # We always upload to project "1" so that tests will be consistent no matter how many / which tests are run
        project_id_1 = "1"
        ...
        # we are uploading 2 projects, so create another one
        project_name_2 = "test_project_nextseq_nml_2"
        project_description_2 = "test_project_description_nextseq_nml_2"
        project_2 = model.Project(name=project_name_2, description=project_description_2)
        test_api.send_project(project_2)
        project_id_2 = "2"

        # Do the upload
        upload_result = upload_run_single_entry(path.join(path_to_module, "fake_nextseq_nml_classic_data"))
        ...
        for sample in sample_list_1:
            if sample.sample_name == "SA20121712a":
                sample_1_found = True
                sequence_files = test_api.get_sequence_files(project_id_1, sample.sample_name)
                self.assertEqual(len(sequence_files), 2)
                res_sequence_file_names = [
                ...
                expected_sequence_file_names = [
                    'SA20121712a_S2_R1_001.fastq.qz',
                    'SA20121712a_S2_R2_001.fastq.qz'
                ]
                self.assertEqual(res_sequence_file_names.sort(), expected_sequence_file_names.sort())

        for sample in sample_list_2:
            if sample.sample_name == "SA20121716a":
                sample_2_found = True
                sequence_files = test_api.get_sequence_files(project_id_2, sample.sample_name)
                self.assertEqual(len(sequence_files), 2)
                res_sequence_file_names = [
                ...
                expected_sequence_file_names = [
                    'SA20121716a_S1_R1_001.fastq.qz',
                    'SA20121716a_S1_R2_001.fastq.qz'
                ]
                self.assertEqual(res_sequence_file_names.sort(), expected_sequence_file_names.sort())
    ...
    def test_valid_nextseq_nml_dual_upload(self):
        ...
        self.write_to_config_file(
            ...
            password=tests_integration.password,
            base_url=tests_integration.base_url,
            parser="nextseq_nml_dual_upload",
            readonly=False
        )
        ...
        # This may not be the project that the files get uploaded to,
        # but one will be made in the case this is the only test being run
        project_name = "test_project_nextseq_nml_dual"
        project_description = "test_project_description_nextseq_nml_dual"
        project = model.Project(name=project_name, description=project_description)
        test_api.send_project(project)
        # We always upload to project "1" so that tests will be consistent no matter how many / which tests are run
        project_id_1 = "1"
        ...
        # we are uploading 2 projects, so create another one
        project_name_2 = "test_project_nextseq_nml_dual_2"
        project_description_2 = "test_project_description_nextseq_nml_dual_2"
        project_2 = model.Project(name=project_name_2, description=project_description_2)
        test_api.send_project(project_2)
        project_id_2 = "2"

        # Do the upload
        upload_result = upload_run_single_entry(path.join(path_to_module, "fake_nextseq_nml_dual_upload_data"))
        ...
        for sample in sample_list_2:
            if sample.sample_name == "SA20121716b":
                sample_2_found = True
                sequence_files = test_api.get_sequence_files(project_id_2, sample.sample_name)
                self.assertEqual(len(sequence_files), 2)
                res_sequence_file_names = [
                ...
                expected_sequence_file_names = [
                    'SA20121716b_S1_R1_001.fastq.qz',
                    'SA20121716b_S1_R2_001.fastq.qz'
                ]
                self.assertEqual(res_sequence_file_names.sort(), expected_sequence_file_names.sort())
    ...
    def test_valid_nextseq_nml_strict_sample_name_matching(self):
        """
        Test a valid nextseq_nml directory for upload from end to end
        :return:
        """
        ...
        # This may not be the project that the files get uploaded to,
        # but one will be made in the case this is the only test being run
        project_name = "test_project_nextseq_nml_strict"
        project_description = "test_project_description_nextseq_nml_strict"
        project = model.Project(name=project_name, description=project_description)
        test_api.send_project(project)
        # We always upload to project "1" so that tests will be consistent no matter how many / which tests are run
        project_id_1 = "1"
        ...
        for sample in sample_list_1:
            if sample.sample_name == "SA20121712b":
                ...
                self.assertEqual(res_sequence_file_names.sort(), expected_sequence_file_names.sort())
            if sample.sample_name == "other-SA20121716b":
                sample_2_found = True
                sequence_files = test_api.get_sequence_files(project_id_1, sample.sample_name)
                self.assertEqual(len(sequence_files), 2)
                res_sequence_file_names = [
                ...
                expected_sequence_file_names = [
                    'other-SA20121716b_S1_R1_001.fastq.qz',
                    'other-SA20121716b_S1_R2_001.fastq.qz'
                ]
                self.assertEqual(res_sequence_file_names.sort(), expected_sequence_file_names.sort())
