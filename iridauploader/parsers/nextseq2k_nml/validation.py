...
def validate_sample_sheet(sample_sheet_file):
    ...
    Requires [BCLConvert_Data] for creating Sample objects and requires
    ...
    if not all([header_sect_found, data_sect_found, all_data_headers_found, reads_sect_found]):
        ...
        if data_sect_found is False:
            v_res.add_error(exceptions.SampleSheetError("[BCLConvert_Data] section not found in SampleSheet",
                                                        sample_sheet_file))
